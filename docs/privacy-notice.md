...
| **去重** | 同一文件内的重复消息自动去重；导入指纹（SHA-256）已落库（跨文件去重规划中） |
...
Ramaria 提供诊断信息导出功能（设置页 → 诊断与更新 → 导出诊断信息），用于故障排查。
...
- CLI 使用指南：`docs/cli-user-guide.md`
- 完整架构说明：`docs/dev/00-architecture/arch-plan.md`
