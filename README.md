...
**规模统计**：8 个 crate、~190 个源文件、~79,000 行 Rust 源码（含测试 ~88,000 行）、1200+ 测试函数。
...
v1.4 是"对话体感地基"版本：让对话回复开始"像 ta"——语气、口癖、原话直接复现；能答上 persona 的细节；隔天回来能接上上次聊的；空闲时长可自行设置。在 v1.3 基础上完成了表达层（utt 原文话语块 + examples 自学习）与脉络层（空闲 UI + 桥接 + 驱动环骨架）实装、evidence_notes 结构化升级，并新增设置功能完善（链接全部配置项，前端分基础/高级两级）。

**v1.4 新增**：

- utt 话语块（L0 原文表达层）：时间间隙 + 条数上限切分（单边合并、无 persona 发言块丢弃）→ 全量/增量构建（幂等去重）→ layer='l0' 向量索引 → 对话时检索注入【原文片段】段落；原文按 persona_uid 严格隔离，白名单默认仅角色类 persona（助手类行为无回归）
- examples 自学习激活：会话封存时自动抽取"对方 → 你"回复对入库（纯规则、查重幂等）→ 评分轮换注入（话题/情绪/长度）→ 记忆检索未命中时作风格兜底
- evidence_notes 结构化线索：`[{text, time?, who?, cause?}]` 对象数组（一次性迁移）+ L2 事件提取消费 cause 因果线索
- 会话桥接：新窗口自动带上一会话尾部原文（utt 块优先，无块降级取末 5 条原文），`bridge_enabled` 可配置开关
- 空闲时长滑动块 UI：5~60 分钟连续滑动（滑到尽头切换自定义输入），保存后热更新不重启
- 四层注入骨架：CRISPE 精简对齐 v3.1 §8.2（角色行为/说话风格/知识/记忆脉络），预算分配器（脉络独立 ≤ 30%），行为/知识槽位预留（v1.5/v1.6 填充）
- 设置功能完善：config.toml 与 DB 双写同步（统一写入口 + 启动一致性校验）+ 基础/高级两级设置页（全部配置项可见可改可恢复默认）
- 修复：真实消息导入测试 6 项 P0/P1 缺陷（会话归属、utt/examples 激活、L1 归属真相源、L3 空数组误降级、孤儿会话、深度处理 LIMIT）；token_budget 多字节截断；Qwen3-Embedding 导入校验
- 测试总数 ≥ 1200（v1.3: 700+，新增 ~500 个，含 v1.3 遗留 2 项集成测试收尾补齐）
...
Python 版位于项目根目录，已进入**维护模式**，不再活跃开发。Rust v1.4 是正式替代版本。
...
v1.4 已完成"对话体感地基"（原文注入 + 生命周期 + 设置完善）。后续按 2.0 路线推进（`docs/dev-2.0/roadmap.md`）：v1.5 行为驱动（反应模式自动复现 + 导入 ETA + 生成缓存 + 探针 CLI 骨架）→ v1.6 知识深化（知识卡片 + 画像升级 + 探针自动评分）→ v1.7 风格与闭环（风格统计 + 反馈环 + 正式评估）→ 2.0 整合发布。

以下功能已在架构中预留接口，进入 **延后（deferred）** 队列，不在 v1.4 发布范围内：
