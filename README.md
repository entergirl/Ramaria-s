...
v1.5 是"行为驱动"版本：让 persona 的反应模式自动复现——聊到特定话题时，ta 遇到工作吐槽时的典型反应、安慰人的方式会自然流露，无需人工编写规则。

**v1.5 新增**：

- 行为规则自动学习：从过往对话中自动归纳"什么情境下 ta 会怎么反应"，聊到相关话题时自动按 ta 的习惯回应；规则可查看、可编辑、可禁用、可手工导入，并能溯源到原始事件
- 导入进度可预期：后台导入时实时显示"已完成 x/y · 预计剩余 N 分钟"
- 生成缓存：重复的生成请求不再重复计费，重跑/重试导入不重复花钱
- 对话上下文感知：跨时段连续聊天时，摘要与回应能体现话题的延续与转折
- 命令行体验升级：统一 JSON 输出、脚本友好、命令命名规范化
- 设置页视觉回归粉蓝双色设计系统
- 测试总数 ≥ 1590（v1.4: ≥1200，新增 ~390 个）
