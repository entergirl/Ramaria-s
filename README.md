...
- 风格统计：五维统计（口癖/句式句长/标点/情感/话题词汇）→ 显著性检验（全局基线池 + 二项 z 检验）→ 自动生成风格规则注入对话，手工说话风格仍可覆盖；数据不足自动跳过
- 长对话不丢信息：渐进式摘要（超长会话分段记忆）+ 跨会话脉络按"时间 × 话题"加权注入（可开关）
- 对话更连贯：检索命中的记忆自动"触碰"刷新，激活近期访问加成
- 自我修正闭环：识别纠正（S2）/ 继续发言（S3）弱信号写入审计日志，积累到阈值标记候选复审，默认不自动改动规则/画像（隐私安全）
- 探针定稿：utt 切分参数与行为聚类参数经正式档位实验定稿写入默认配置（θ_gap=10/条数 80/top_k=3；θ_nb=0.65/β1=0.85/β2=0.10）
- 四要素消融评估：新增 emotion 维度、`--repeat` 逐轮聚合、消融档位与对比报告（`probe report --ablation`）；正式评估报告交付（结论见 docs/dev-1.7）
- 工程加固：probe 可复算性（`--repeat`）、`--no-rebuild-utt`、CLI 中文路径修复、exit code 4 修正、JSON 截断解析修复、CUDA 构建文档化（不新增 py/cmd 文件）
- 测试总数继续增长（分 crate 全绿，全量回归由负责人验收）

### v1.6.0 — 2026-08-26

"知识深化"版本：把记忆里散落的碎片自动归纳成结构化知识卡片，对话中问及 ta 的相关事实时按需注入，越了解 ta、答得越准。主要新增：知识层（事实自动抽取/判重/分层/版本链仲裁，记忆页「知识」标签只读查看历史版本）、画像升级（跨版本匹配阈值对齐、冷启动跨用户校准、降级置信度可解释）、隐私与降级（embedding/LLM 不可用时静默降级、日志脱敏不记原文）、探针自动评分（`probe evaluate/report`）、命令行（`fact list/show`、导入 `--side`）。注意：migrations 合并为单基线 + `persona_facts` 版本化直建，为**破坏性变更，旧库需重建**。测试总数继续增长（分 crate 全绿，全量回归由负责人验收）。
