...
每条 evidence 应记录"表达了什么观点/经历了什么事件"的原始描述，
引用原话或转述具体事实，不少于 5 个中文字符。
可以包含时间、地点、人物等具体信息。

【其他字段说明】
- summary：只记结论，不记过程；用会话中的实际人物名称指代；客观陈述，不加主观评价
- keywords：只取中文名词，不取动词或形容词；避免同义词重复；无合适关键词时填空字符串
...
【其他字段说明】
- summary：只记结论，不记过程；用会话中的实际人物名称指代；客观陈述，不加主观评价
- keywords：只取中文名词，不取动词或形容词；避免同义词重复；优先从下方候选列表中选择
- time_period：根据对话发生的时间判断；若无法判断，根据内容氛围推断
- atmosphere：优先反映整体基调；四字以内，不超过四字
...
mod tests {
    ...
    fn prompt_contains_all_required_fields() {
        ...
        assert!(
            prompt.contains("表示最近一个月每天加班到10点以后"),
